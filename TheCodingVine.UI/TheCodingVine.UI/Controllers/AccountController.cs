...
namespace TheCodingVine.UI.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            //loginVM.SetStaticLinks(siteLinks);
        ...
        {
            var verifyCodeVM = new VerifyCodeViewModel { Provider = provider, ReturnUrl = returnUrl, RememberMe = rememberMe };
            BlogManager manager = BlogManagerFactory.Create();
            var siteLinks = manager.GetSiteLinks();
        ...
        public async Task<ActionResult> EditUser(string id)
        {
            ...
            editVm.Roles = RoleManager.Roles.Select(r => new SelectListItem { Text = r.Name, Value = r.Id }).ToList();
        ...
        public async Task<ActionResult> EditUser(EditUserViewModel model)
        {
            ...
            var userToEdit = UserManager.FindById(model.User.Id);

            List<string> roleNames = new List<string>();

            foreach (var role in userToEdit.Roles)
            {
            ...
            {
                IdentityResult deletionResult = UserManager.RemoveFromRole(model.User.Id, name);
            }

            var updatedRoleName = RoleManager.FindById(model.SelectedRoleId).Name;
            var updatedPassword = model.NewPassword;
            var updateduserName = model.User.Email;

            UserManager.AddToRole(userToEdit.Id, updatedRoleName);
            UserManager.AddPassword(userToEdit.Id, updatedPassword);
            UserManager.SetEmail(userToEdit.Id, updateduserName);
		...
		public ActionResult DeleteUser(string id)
		{
			...
			var availRoles = RoleManager.Roles;

			userVM.SetUser(userToDelete);
			userVM.SetUsersRoles(availRoles);
		...
		public ActionResult AllUsers()
		{
			...
			usersVM.SetUserList(users);
			usersVM.SetRolesList(roles);
